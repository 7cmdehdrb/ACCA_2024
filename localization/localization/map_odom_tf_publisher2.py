...
class Map_Odom_TF_Publisher(Node):
    def __init__(self):
        ...
        class Odom_State(object):
            ...
            def update_state(self, new_pose):
                ...
                _, _, new_yaw = euler_from_quaternion(
                    [new_quat.x, new_quat.y, new_quat.z, new_quat.z]
                )

                last_quat = self.last_pose.pose.pose.orientation
                _, _, last_yaw = euler_from_quaternion(
                    [last_quat.x, last_quat.y, last_quat.z, last_quat.z]
                )
                ...
                pose_difference_norm = np.linalg.norm(pose_difference_vector, ord=1)
        ...
        self.params = self.declare_parameters(
            ...
            parameters=(
                ...
                ("angle_difference_threshold", 1.0),
                ("score_threshold", 3.0),
                ("logging", True),
            ),
        )
        ...
        self.angle_difference_threshold = (
            self.get_parameter("angle_difference_threshold")
            .get_parameter_value()
            .double_value
        )
        ...
        self.get_logger().info(
            "linear_err_threshold: {}\nangular_err_threshold: {}\nangle_difference_threshold: {}\nscore_threshold: {}".format(
                self.linear_err_threshold,
                self.angular_err_threshold,
                self.angle_difference_threshold,
                self.score_threshold,
    ...
    # String getter
    def get_data(self):
        return "linear: {}\tangular: {}\angle_difference_err: {}\tscore: {}".format(
            round(self.linear_err, 3),
            round(self.angular_err, 3),
            round(self.angle_difference_err, 3),
            round(self.score, 3),
        )
    ...
    def update(self):
        ...
        data = [self.linear_err, self.angular_err, self.angle_difference_err]

        self.err_pub.publish(Float32MultiArray(data=data))
        ...
        if (
            ...
            and abs(self.angle_difference_err) < self.angle_difference_threshold
