...
class Drive():
    ...
    def publish_cmd(self):
        ...
        if path_type == "global":
            ...
            last_target_idx = 0
        steer, target_idx, self.hdr, self.ctr, self.h, self.c= self.st.stanley_control(self.state, self.path.cx, self.path.cy, self.path.cyaw, last_target_idx)
    ...
    def make_txt(self):
        self.count += 1
        f = open("/home/gjs/stanley_test/he_gain:{}, ce_gain:{}, he_thr:{}, ce_thr:{}_2"
                    .format(self.ss.he_gain, self.ss.ce_gain, self.ss.he_thr, self.ss.ce_thr), 'a')
            
        data = "{}, {}, {}, {}, {}, {}, {}\n".format(self.h, self.c, self.hdr, self.ctr, self.current_speed * 3.6, self.adapted_speed, self.steer)
        if self.count >=300:
            f.write(data)
        else:
            f.close()
